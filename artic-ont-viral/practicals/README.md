...
2. Begin an `interactive` session with 2 `cpus` on either `compute05` or `compute06`
3. Organize your project directory structure as indicated below. 

    <img src="../img/dir-exercise.png" alt="Project directory structure"
    style="height: px; width:27212px;"/>
4. Create `soft` or `symbolic` links to the required directories
5. Load all the required modules that will be utilized in the analysis steps
6. Retrieve the reference genome sequence from the NCBI database and output it
   in the `genomes` directory
7. The raw data in `fastq` format can be retrieved from the European Nucleotide
...
8. Download reference genome file in FASTA format using your preffered method.
 
9. For use in variant calling and visualization of alignment, please create the
...
10. Perform a quality assessment of the raw reads in `fastq` format and output
...
11. Perform a preprocessing step to only select viable reads for subsequent
...
12. Perform reference based read alignment to the reference genome and output
...
13. Perform a post-alignment process to assign each raed to its derived
...
14. For each primer pool, perform variant calling to while using the given model
...
15. Compute depths for each primer pool and mask low depth regions to generate a
    preconsensus file
    >**<strong style="color:magenta;opacity: 0.80 ">Quiz:</strong>**
    - What is the highest depth for each primer pool?

16. Generate consensus sequence and rename the header of the resulting consensus
    file
...
17. Compute non-normalized read depth per position and plot the output using the
    utility Rscript provided

18. In the alignment directoty, concatenate the reference genome file, the
...
19. Perform a multiple sequence alignment and generate a maximul likelihood
    phylogenetic tree
    
20. Identify the serotype and genotype of the sample using the typing tool available at
https://www.genomedetective.com/app/typingtool/dengue/

21. Transfer the outputs from the `hpc` to your `local` PC and visualize the
